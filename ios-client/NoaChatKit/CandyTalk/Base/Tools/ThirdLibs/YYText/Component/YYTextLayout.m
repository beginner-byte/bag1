            ...
            for (NSUInteger r = 0; r < runCount; r++) {
                ...
                for (NSUInteger g = 0; g < glyphCount; g++) {
                    ...
                    if (isColorGlyph) {
                        ...
                        NSString *glyphStr = [layoutStr safeSubstringWithRange:NSMakeRange(runStrIdx[g], runStrLen)];
...
- (YYTextPosition *)closestPositionToPoint:(CGPoint)point {
    ...
    if (line.range.length <= 2) {
        NSString *str = [_text.string safeSubstringWithRange:line.range];
    ...
    if (verticalMove) { // up/down in text layout
        ...
        if (afinityEdge) {
            if (pos == insideLine.range.location + insideLine.range.length) {
                NSString *subStr = [_text.string safeSubstringWithRange:insideLine.range];
