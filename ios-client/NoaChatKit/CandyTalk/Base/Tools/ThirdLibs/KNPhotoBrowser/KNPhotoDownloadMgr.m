...
@interface KNPhotoDownloadMgr() <NSURLSessionDelegate> {
...
#ifdef DEBUG
        DLog(@"%lld-%lld == > %f",totalBytesWritten,totalBytesExpectedToWrite,progress);
#endif
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    ...
    NSString *pathExtension = _item.url.pathExtension;
    if ([NSString isNil:pathExtension]) {
        pathExtension = @"mp4";
    }
    NSString *file = [_filePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.%@",[self md5:_item.url.lastPathComponent.stringByDeletingPathExtension],pathExtension]];
...
- (BOOL)startCheckIsExistVideo:(KNPhotoItems *)photoItems {
    ...
    if (!(isDir && existed)) {
        ...
        NSString *pathExtension = photoItems.url.pathExtension;
        if ([NSString isNil:pathExtension]) {
            pathExtension = @"mp4";
        }
        NSString *path = [_filePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.%@",[self md5:photoItems.url.lastPathComponent.stringByDeletingPathExtension],pathExtension]];
...
- (void)removeVideoByURLString:(NSString *)urlString{
    ...
    if ((isDir && existed)) {
        ...
        NSString *path = [_filePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.%@",[self md5:urlString.lastPathComponent.stringByDeletingPathExtension],pathExtension]];
