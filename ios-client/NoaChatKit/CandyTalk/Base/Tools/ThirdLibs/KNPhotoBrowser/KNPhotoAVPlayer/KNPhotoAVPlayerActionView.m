...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (_dismissImgView && !_dismissImgView.hidden && _dismissImgView.alpha >= 0.01 && _dismissImgView.isUserInteractionEnabled) {
        CGRect convertedFrame = [_dismissImgView convertRect:_dismissImgView.bounds toView:self];
        CGRect expandedFrame = CGRectInset(convertedFrame, -20, -20);
        if (CGRectContainsPoint(expandedFrame, point)) {
            return _dismissImgView;
        }
    }
