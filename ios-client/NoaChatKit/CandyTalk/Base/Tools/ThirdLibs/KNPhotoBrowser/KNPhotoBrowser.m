...
@property (nonatomic, strong) SyncMutableArray *followArr;
...
- (SyncMutableArray *)followArr{
    if (!_followArr) {
        _followArr = [[SyncMutableArray alloc] init];
    }
    return _followArr;
}
        ...
        for (NSInteger i = 0; i < _itemsArr.count; i++) {
            if(i != _currentIndex){
                ...
                if(items.url != nil && [items.url hasPrefix:@"http"] && items.isVideo == false){
                    //地址中文的处理
                    NSString *urlTempString = [[NSString stringWithFormat:@"%@", items.url] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]];
                    NSURL *tempUrl = [NSURL URLWithString:urlTempString];
                    if (tempUrl) {
                        [urlArr addObject:tempUrl];
                    }
                }
            }
        }
...
//        _currentIndex = fabs(scrollView.contentOffset.x) / _layout.itemSize.width;
...
- (void)panDidGesture:(UIPanGestureRecognizer *)pan{
    ...
    if (items.isVideo) {
        ...
        if (_isNeedOnlinePlay) {
            ...
            playerView = (KNVideoIjkPlayerView *)cell.locatePlayerView;
            location    = [pan locationInView:[(KNVideoIjkPlayerView *)playerView playerBgView]];
        }
        ...
        case UIGestureRecognizerStateBegan:{
            ...
            if(items.isVideo){
                if (_isNeedOnlinePlay) {
                    ...
                    _startFrame = [(KNVideoIjkPlayerView *)playerView playerBgView].frame;
                    [(KNVideoIjkPlayerView *)playerView setIsNeedVideoPlaceHolder:![(KNVideoIjkPlayerView *)playerView isBeginPlayed]];
                    [[(KNVideoIjkPlayerView *)playerView playerView] setBackgroundColor:UIColor.clearColor];
                }
        ...
        case UIGestureRecognizerStateChanged:{
            ...
            if(items.isVideo){
                if (_isNeedOnlinePlay) {
                    ...
                    [(KNVideoIjkPlayerView *)playerView playerView].frame         = CGRectMake(x, y, width, height);
                    [(KNVideoIjkPlayerView *)playerView playerLayer].frame        = CGRectMake(0, 0, width, height);
                    [(KNVideoIjkPlayerView *)playerView placeHolderImgView].frame = CGRectMake(x, y, width, height);
                }
            ...
            if (items.isVideo) {
                if(fabs(point.y) > 200 || fabs(velocity.y) > 500){
                    ...
                    if (_isNeedOnlinePlay) {
                        ...
                        _startFrame = [(KNVideoIjkPlayerView *)playerView playerView].frame;
                    }
                    ...
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(PhotoBrowserAnimateTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        [[(KNVideoIjkPlayerView *)playerView playerView] setBackgroundColor:UIColor.blackColor];
                        [playerView setIsNeedVideoPlaceHolder:true];
                    });
                }
            }else {
...
- (void)cancelVideoAnimation:(id)playerView{
    ...
    [UIView animateWithDuration:PhotoBrowserAnimateTime animations:^{
        if (self.isNeedOnlinePlay) {
            ...
            [(KNVideoIjkPlayerView *)playerView playerView].frame = frame;
            [(KNVideoIjkPlayerView *)playerView playerLayer].frame = frame;
            [(KNVideoIjkPlayerView *)playerView placeHolderImgView].frame = frame;
        }
    } completion:^(BOOL finished) {
        self.view.backgroundColor = [UIColor blackColor];
    }];
...
- (void)dismiss{
    ...
    for (int i = 0; i < self.followArr.count; i++) {
        UIView *subView = self.followArr.safeArray[i];
        [subView setAlpha:1];
    }
...
- (void)customViewSubViewsWillShow{
    if ([self isEmptyArray:self.followArr.safeArray] == false) {
        for (int i = 0; i < self.followArr.count; i++) {
            UIView *subView = self.followArr.safeArray[i];
            [subView setAlpha:1];
        }
...
- (void)photoBrowserWillDismissWithAnimated:(UIImageView *)tempView items:(KNPhotoItems *)items{
    ...
    if([self isOutOfScreen:rect]){
        if(isPortrait == true){
            [UIView animateWithDuration:PhotoBrowserAnimateTime delay:(int64_t)(0.3 * NSEC_PER_SEC) options:UIViewAnimationOptionCurveEaseOut animations:^{
                [tempView setAlpha:0.f];
            } completion:^(BOOL finished) {
                [tempView removeFromSuperview];
                [self showStatusBar];
...
- (void)layoutCollectionViewAndLayout{
    ...
    [_numView setFrame:(CGRect){{0, y}, {PBViewWidth, 30}}];
    [_pageControl setFrame:(CGRect){{0, PBViewHeight - 50}, {PBViewWidth,30}}];
    [_operationBtn setFrame:(CGRect){{PBViewWidth - 35 - 15 - x, y}, {30,30}}];
...
- (void)deviceWillOrientation{
    ...
    if(![item.url.lastPathComponent.pathExtension.lowercaseString isEqualToString:@"gif"] && item.isVideo == false){
        ...
        NSString *urlTempString = [[NSString stringWithFormat:@"%@", url] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]];
        [_imageView imageWithUrl:[NSURL URLWithString:urlTempString]
    ...
    if(imageView.image == nil){
        if (items.isVideo == false) {
            if (items.url) {
                ...
                [imageView sd_setImageWithURL:[items.url getImageFullUrl] placeholderImage:DefaultImage options: SDWebImageAllowInvalidSSLCertificates];
            }else {
            ...
            if (items.videoPlaceHolderImageUrl) {
                //有缩略图
                CGSize size = CGSizeMake(PBViewWidth, PBViewWidth);
                [imageView sd_setImageWithURL:[items.videoPlaceHolderImageUrl getImageFullUrl] placeholderImage:DefaultImage options: SDWebImageAllowInvalidSSLCertificates];
            }else {
                imageView.image = [self createImageWithUIColor: UIColor.clearColor size: CGSizeMake(PBViewWidth, PBViewWidth)];
            }
        }
...
#ifdef DEBUG
    DLog(@"currentPage ===> %zd", pageControl.currentPage);
#endif
