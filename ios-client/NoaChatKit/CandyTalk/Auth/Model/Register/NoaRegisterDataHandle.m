...
/// 处理注册成功并保存当前账号的设备登录凭据。
/// @param data 注册成功响应，包含用户信息及当前设备凭证。
/// @param account 用户注册时输入的账号。
/// @param loginType 注册账号类型，用于保存上次登录账号。
/// @param password 用户注册时输入的明文密码，仅交给现有弱密码检查流程使用。
- (void)handleRegisterSuccess:(id)data
    ...
    // 注册成功后保存账号对应的用户 UID 和设备凭证，供该账号下次登录使用。
    ...
    [NoaDeviceLoginCredential saveCredentialWithLoginInfo:account
                                                  userUid:registerUserModel.userUID
