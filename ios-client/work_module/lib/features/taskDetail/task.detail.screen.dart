...
class TaskDetailScreen extends StatelessWidget with ScreenMixin {
        ...
        child: ConstrainedBox(
          ...
          child: ListView(
            ...
            children: [
              ...
              if (Get.find<TaskDetailController>().canAddAssignees) ...[
                SizedBox(height: 10.h),
                Align(
                  alignment: Alignment.centerRight,
                  child: OutlinedButton.icon(
                    onPressed: Get.find<TaskDetailController>().addAssignees,
                    icon: const Icon(Icons.person_add_alt_1_outlined),
                    label: Text(S.of(context).taskDetailAddAssignees),
                  ),
                ),
              ],
              ...
              if (_isCreator(task)) ...[
                ...
                OutlinedButton.icon(
                  ...
                  label: Text(S.of(context).taskDetailDelete),
                ),
              ],
            ],
          ),
  ...
  bool _isCreator(TaskItem task) {
    return task.creatorId.isNotEmpty &&
        task.creatorId == Get.find<WorkSessionService>().workerUserId;
  }

  /// 二次确认后执行 Worker 删除、原生解散和服务端完成回调。
  Future<void> _confirmDelete(BuildContext context, TaskItem task) async {
    final deleteSuccessMessage = S.of(context).taskDetailDeleteSuccess;
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child: Text(S.of(context).taskDetailCancel),
          ),
          FilledButton(
            onPressed: () => Navigator.of(dialogContext).pop(true),
            child: Text(S.of(context).taskDetailConfirmDelete),
          ),
        ],
      ),
    );
    ...
    try {
      ...
      final command = await repository.beginDeleteTask(task.id);
      if (command.action == 'dissolve_group') {
        await Get.find<WorkHostBridge>().dissolveTaskGroup(command.groupId);
        await repository.completeTaskGroupDissolution(
          groupId: command.groupId,
          operationId: command.operationId,
        );
      }
      EasyLoading.showSuccess(deleteSuccessMessage);
      Get.back(result: task.id);
    } catch (error) {
      EasyLoading.showError(error.toString());
    }
  }
